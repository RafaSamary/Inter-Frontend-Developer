...
import { sign } from 'jsonwebtoken'; // LIB DO TOKEN JWT
import md5 from 'crypto-js/md5';

import {User} from '../../entity/User';
import AppError from '../../shared/error/AppError';
import authConfig from '../../config/auth';  // IMPORTANDO AS CONFIGURAÇÕES DO  TOKEN
...
export default class UserService {
    ...
    async signin(user: UserSignIn) {
       ...
       const existUser = await userRepository.findOne({where: {email, password: passwordHash}}) // CHAMADO AO BANCO E VERIFICAÇÃO

       if(!existUser){
         throw new AppError('Usuário não encontrato', 401);
       }

       const { secret, expiresIn } = authConfig.jwt;  // CRIANDO O TOKEN JWT

       const token = sign({                            // INFORMAÇÕES PARA O  TOKEN
